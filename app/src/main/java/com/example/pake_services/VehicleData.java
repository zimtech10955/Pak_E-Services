...
//       createButtonsLinks();
    ...
    private void incresecount() {
        Verabls.count++;
      //  Toast.makeText(VehicleData.this, ""+Verabls.count, Toast.LENGTH_SHORT).show();
    }

    void startSite(Button btn) {
        String key = btn.getText().toString();
        try {
            database.getReference().child("Vehicle Data").child(key)
                    .addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            try{WebModel webModel = snapshot.getValue(WebModel.class);
                                String url = webModel.getLink();
//                            Toast.makeText(VehicleData.this, "URL: " + url, Toast.LENGTH_SHORT).show();
                                Intent intent = new Intent(getApplicationContext(), WebLauncher.class);
                                intent.putExtra("site", url);
                                startActivity(intent);}
                            catch (Exception e){
                                Toast.makeText(VehicleData.this, "Sorry! This Feature cannot be accessed at this moment", Toast.LENGTH_SHORT).show();
                            }
                        }
                        @Override
   ...
   void createButtonsLinks(){
       ...
       try {
           for (int i = 0; i < 4; i++) {
               database.getReference().child("Vehicle Data")
                       .child(buttons.get(i).getText().toString())
                       .setValue(models.get(i)).addOnSuccessListener(new OnSuccessListener<Void>() {
                   @Override
                   public void onSuccess(Void aVoid) {
                       Toast.makeText(VehicleData.this, "Added Successfully", Toast.LENGTH_SHORT).show();
                   }
               });
           }
       } catch (Exception e) {
...
//   237530441306988_237532874640078
        ...
        InterstitialAdListener interstitialAdListener = new InterstitialAdListener() {
            ...
            public void onError(Ad ad, AdError adError) {
                ...
                Toast.makeText(VehicleData.this, "Please Connect to Internet first", Toast.LENGTH_LONG).show();
